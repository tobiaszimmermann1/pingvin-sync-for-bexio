...
import { Table, Thead, Tbody, Tr, Th, Td, TableContainer, Flex, ChakraProvider, Button, Spinner, Stack, Text, Link } from "@chakra-ui/react"
...
function Orders() {
  ...
  useEffect(() => {
    ...
    apiCall("GET", `orders`, { offset, page_size: pageSize })
      .then(res => {
        if (!isGetOrders) {
          return
        }
                                ...
                                <Stack gap="1">
                                  <Flex flexDirection="row" alignItems="center" gap="1" justifyContent="flex-start">
                                    ...
                                    <Text fontSize="xs" mt="0" mb="0">
                                      {__("Zuletzt synchronisiert am", "pingvin-sync-for-bexio")} {order.pvbexio_sync && order.pvbexio_sync.last_sync ? new Date(order.pvbexio_sync.last_sync).toLocaleString() : __("Unbekannt", "pingvin-sync-for-bexio")}
                                    </Text>
                                  </Flex>
                                  <Link href={`https://office.bexio.com/index.php/kb_order/show/id/${order.bexio_order_id}`} isExternal>
                                    <Flex flexDirection="row" alignItems="center" gap="1" justifyContent="flex-start">
                                      ...
                                      <Text fontSize="xs" mt="0" mb="0">
                                        {order.bexio_order_nr ? `${__("Bexio Auftrag:", "pingvin-sync-for-bexio")} ${order.bexio_order_nr}` : __("Kein Bexio Auftrag", "pingvin-sync-for-bexio")}
                                      </Text>
                                    </Flex>
                                  </Link>
                                  {/*}
                                  <Flex flexDirection="row" alignItems="center" gap="1" justifyContent="flex-start">
                                    ...
                                    <Text fontSize="xs" mt="0" mb="0">
                                      {order.bexio_order_status ? `${__("Bexio Status:", "pingvin-sync-for-bexio")} ${order.bexio_order_status}` : __("Kein Bexio Status", "pingvin-sync-for-bexio")}
                                    </Text>
                                  </Flex>
                                  */}
                                </Stack>
