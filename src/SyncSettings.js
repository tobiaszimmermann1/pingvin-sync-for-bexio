...
function SyncSettings( { setSettingsInterval } ) {
	...
	const showToast = ( success, message ) => {
		toast( {
			title: success
				? __( 'Settings successfully updated.', 'pingvin-bexio-sync' )
				: __( 'An error occurred', 'pingvin-bexio-sync' ),
	...
	const saveSettings = ( type, enabled, interval ) => {
		apiCall( 'POST', 'syncSettings', { type, enabled, interval } ).then(
			( res ) => {
				...
				<Stack>
					...
					<Stack direction="column" gap="4">
						...
						<Text fontSize="sm" mt="-4" fontStyle="italic">
							{ __(
								'Wenn du die Synchronisierung deaktivierst, werden alle geplanten Synchronisierungen abgebrochen.',
								'pingvin-bexio-sync'
							) }
						</Text>
						...
						<Text fontSize="sm" mt="-4" fontStyle="italic">
							{ __(
								'Wenn du das Intervall änderst, wird die nächste Synchronisierung abgebrochen und entsprechend dem gewählten Intervall neu geplant.',
								'pingvin-bexio-sync'
							) }
						</Text>
					</Stack>
